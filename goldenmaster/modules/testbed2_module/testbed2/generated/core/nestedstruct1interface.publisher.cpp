...
private:
    ...
    std::vector<std::reference_wrapper<INestedStruct1InterfaceSubscriber>> AllChangesSubscribers;
 ...
 */
void NestedStruct1InterfacePublisherImpl::subscribeToAllChanges(INestedStruct1InterfaceSubscriber& subscriber)
{
    ...
    {
        AllChangesSubscribers.push_back(std::reference_wrapper<INestedStruct1InterfaceSubscriber>(subscriber));
    }
}

void NestedStruct1InterfacePublisherImpl::unsubscribeFromAllChanges(INestedStruct1InterfaceSubscriber& subscriber)
{
    auto found = std::find_if(AllChangesSubscribers.begin(), AllChangesSubscribers.end(),
                        [&subscriber](const auto element){return &(element.get()) == &subscriber;});
    if (found != AllChangesSubscribers.end())
