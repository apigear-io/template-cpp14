...
{
public:
    TestService(std::shared_ptr<Test::Api::ITestApi> TestApi, ApiGear::ObjectLink::RemoteRegistry& registry)
        :TestApiService(TestApi, registry)
    {}

    void olinkLinked(const std::string& objectId, ApiGear::ObjectLink::IRemoteNode* node) override
    {
        Test::Api::olink::TestApiService::olinkLinked(objectId, node);
        m_status = ServiceStatus::LinkedRceived;
    }

    void olinkUnlinked(const std::string& objectId) override
    {
        Test::Api::olink::TestApiService::olinkUnlinked(objectId);
